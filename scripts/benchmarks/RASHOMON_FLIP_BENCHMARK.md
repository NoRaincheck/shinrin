...
combination. The joint search is best-first (uniform-cost) over partial
combinations with canonical-state dedup and node/time budgets; budget
exhaustion yields "best found, optimality unproven", while an emptied queue
...
| | spotset/rashomon | **100%** | 0% | 0% | **4.60** | 4.60 | **640** | **0.005s** |
| | rf64/ref | 100% | 0% | 0% | 3.34 | 1.70 | 320 | 0.118s |
| | rf64/rashomon | **10%** | 0% | **90%** | – | – | 8,624,300 | 395s |
| compas (197-tree set) | spot/ref | 100% | 0% | 0% | 1.00 | 1.00 | 120 | 0.001s |
| | spotset/ref | 100% | 0% | 0% | 1.00 | 1.00 | 80 | 0.024s |
| | spotset/rashomon | **100%** | 0% | 0% | **4.63** | 4.63 | **30,908** | **0.346s** |
| | rf64/ref | 100% | 0% | 0% | 0.53 | 1.05 | 1,200 | 0.178s |
| | rf64/rashomon | 100% | 0% | 0% | 2.54 | 5.08 | 286,239 | 12.497s |
...
1. **The Rashomon-robust tweak is cheap and always found.** Flipping every
   ...
   milliseconds to sub-seconds (640–31k search nodes total). The shared
   structure hypothesis holds: near-optimal trees disagree on few paths, so
   one small intersection of leaf constraints flips them all at once.
2. **Robustness costs little on sparse trees but is real.** Mean distance
   grows from ~1.6 to 4.6 (breast cancer) and 1.0 to 4.6 (compas) toggled
...
3. **Random forests struggle with the identical query.** On breast cancer,
   rf64/rashomon solved only 10% of samples within budget (90% unresolved
   after 500k nodes / 10 s each; 8.6M nodes burned overall), versus 100%
...
4. **Binary-feature data softens the forest case** (compas: RF solved, d
   0.53 -> 2.54, high pre-tweak agreement 0.43). Difficulty tracks feature
   richness and tree diversity rather than tree count alone.
5. Caveat: rf64/rashomon distances are omitted for breast cancer because
   the 10% of "solved" samples are a heavily biased subset (the easy ones);
   the honest signal there is the budget-exhaustion rate.

## Beyond this benchmark: scope="ensemble"

The same machinery also powers `scope="ensemble"`, which flips any sklearn
tree model's own aggregated prediction using its true decision rule — soft
probability votes for forests/trees, AdaBoost's weighted hard votes, and
score-sign crossing over stage outputs for binary GradientBoosting /
HistGradientBoosting — always re-verified against `estimator.predict`.
...
search = RashomonFlipSearch(fitted_model)
results = search.search(X, target=1, scope="ensemble")  # or "reference"/"rashomon"
summary = summarize_flip_results(results)
```

Works with any tree-like classifier: SPOT, SPOTSET, sklearn decision
trees/forests/bagging (soft probability votes), AdaBoost committees
(weighted hard votes), and binary GradientBoosting /
HistGradientBoosting (score-sign crossing over stage outputs). Scopes:
``"reference"`` = first/optimal tree only; ``"rashomon"`` = every member of
the set; ``"ensemble"`` = the estimator's own aggregated prediction,
always verified against its real ``predict``. Guarantees for the first two
are per individual tree — single-tree tweaking is not robust in ensembles,
which is what makes SPOT (one optimal tree) the ideal host for it.

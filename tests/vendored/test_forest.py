...
# Match the original (removed) Boston dataset size so that the tight
# training-reproduction assertions hold: the upstream mondrian code routes
# a sample to the left when its feature value equals a split threshold in
# `predict` but to the right in `extend` (strict `<`), and exact f32
# boundary collisions become likely on the full 20k-sample scaled dataset.
rng = np.random.RandomState(42)
_idx = rng.permutation(boston_X.shape[0])[:506]
boston_X, boston_y = boston_X[_idx], boston_y[_idx]
# The time of split and feature chosen for splitting are highly
# scale-sensitive.
scaler = MinMaxScaler()
X, y = boston_X, boston_y
...
def check_decision_path(ensemble):
    ...
    for sample_ind, curr_leaf in enumerate(leaf_indices):
        ...
        assert np.all(np.isin(curr_leaf, sample_indices))
...
def check_proba_classif_convergence(est, X_train, y_train):
    ...
    # For mondrian forests the far-away convergence to the empirical
    # distribution is governed by the random root split time (tau), which
    # can be small, so we assert convergence at 2 decimals rather than 3.

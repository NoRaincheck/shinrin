"""Tests for the vendored pycorels (CORELS) classifier.

Ported from pycorels/tests/test_corelsclassifier.py and adapted to the
shinrin namespace.
"""
...
from shinrin._corels import CorelsClassifier as C
from shinrin._corels import RuleList, load_from_csv

compas_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "data", "compas.csv"
)
...
def test_fit_Xy():
    ...
    invalid_type = [("wrong_type", [1, 0]), ([[1, 0]], "wrong_type"), (1, 1)]
...
def test_general_params():
    # Format: name, type, default_value, valid_values, invalid_values,
    # invalid_type_value
    params = [
        ...
        (
            ...
            str,
            "lower_bound",
            ["bfs", "curious", "lower_bound", "objective", "dfs"],
            ["yay", "asdf"],
            4,
        ),
...
def test_general_other():
    ...
    data_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "data", "data_train.csv"
    )
...
def test_loadcsv(tmp_path):
    fname = tmp_path / "data.csv"
    fname.write_text("a,b,c,label\n1,0,1,1\n0,1,0,0\n")

    X, y, features, prediction_name = load_from_csv(str(fname))
...
def test_repeated_fits_and_predicts():
    """The native fit state must survive repeated fit/predict cycles."""
    ...
    c = C(verbosity=[], c=0.001)
    for _ in range(3):
        assert c.fit(X, y).score(X, y) > 0.9
        preds = c.predict(X[:8])

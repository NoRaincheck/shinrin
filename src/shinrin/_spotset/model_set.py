"""Container over the SPOTSET Rashomon set extracted by the native engine.

Adapted from treeFARMS' ``treefarms/model/model_set.py`` (BSD-3-Clause,
https://github.com/ubc-systopia/treeFARMS), renamed SPOTSET in this project.
Deviations: dropped the tqdm dependency and replaced invalid
``raise "<string>"`` statements with real exceptions.
"""
...
class ModelSetContainer:
    """Lazily decodes individual trees out of the engine's compact storage."""
    ...
    def to_trie(self):
        trie = {}
        for i in range(self.get_tree_count()):
    ...
    def get_tree_at_idx_raw(self, idx):
        for entry in self.available_metrics["metric_pointers"]:
            ...
            idx -= count
        raise IndexError("Index exceeds total stored trees")

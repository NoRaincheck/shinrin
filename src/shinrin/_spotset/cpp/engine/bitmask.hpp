...
namespace std {
    template <>
    struct hash< spotset::Bitmask > {
        std::size_t operator()(spotset::Bitmask const & bitmask) const { return bitmask.hash(); }
    };

    template <>
    struct less< spotset::Bitmask > {
        bool operator()(spotset::Bitmask const & left, spotset::Bitmask const & right) const { return left < right; }
    };

    template <>
    struct equal_to< spotset::Bitmask > {
        bool operator()(spotset::Bitmask const & left, spotset::Bitmask const & right) const { return left == right; }
    };
}

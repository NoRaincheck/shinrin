...
namespace std {
    template <>
    struct hash< spotset::Model > {
        std::size_t operator()(spotset::Model const & model) const {
            return model.hash();
        }
    };
    ...
    template <>
    struct equal_to< spotset::Model * > {
        bool operator()(spotset::Model * const left, spotset::Model * const right) const { return (* left) == (* right); }
    };
}

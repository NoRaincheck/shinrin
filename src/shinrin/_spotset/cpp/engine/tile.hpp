...
namespace std {
    template <>
    struct hash< spotset::Tile > {
        std::size_t operator()(spotset::Tile const & tile) const { return tile.hash(); }
    };

    template <>
    struct equal_to< spotset::Tile > {
        bool operator()(spotset::Tile const & left, spotset::Tile const & right) const { return left == right; }
    };
}

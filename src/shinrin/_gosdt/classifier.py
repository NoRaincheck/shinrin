...
class GOSDTClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y, y_ref=None, input_features=None, cost_matrix=None, feature_map=None):
        ...
        if y_ref is not None:
            ...
            reference_LB = True  # set flag to indicate using reference LB (needed for correct scope behaviour)
        ...
        if len(self.classes_) == 1:
            ...
            self.trees_ = [
                Tree(json_result[0], self.feature_names_, self.n_classes_, self.classes_)
            ]
        ...
        # Create the binarized [X | y] boolean matrix consumed by the engine
        label_binarizer_ = LabelBinarizer()
        y_onehot = label_binarizer_.fit_transform(y) > 0.5
        if y_onehot.shape[1] == 1:
            y_onehot = np.concatenate((~y_onehot, y_onehot), axis=1)
        input_matrix = np.ascontiguousarray(
            np.concatenate((X, y_onehot), axis=1).astype(np.uint8)
        )

        # Create the cost matrix (float32, C-contiguous)
        costs = np.ascontiguousarray(
            self.__create_cost_matrix(y_bin, cost_matrix).astype(np.float32)
        )

        # Binarized reference labels for the reference-ensemble bound
        reference_labels = None
        if y_ref is not None:
            reference_labels = self.__create_reference_labels(label_binarizer_, y_ref)

        # If the debug flag is set, save the raw inputs for inspection.
        if self.debug:
        ...
        result = gosdt_fit(
            ...
            xy=input_matrix,
            costs=costs,
            feature_map=[sorted(int(i) for i in s) for s in feature_map],
            reference=reference_labels,
        )

        # Wrap the plain dict into an object exposing upstream's attribute names.
        from types import SimpleNamespace
        ...
        json_result = json.loads(self.result_.model)
        self.trees_ = [
            Tree(res, self.feature_names_, self.n_classes_, self.classes_)
    ...
    def __create_cost_matrix(self, y, custom_cost_matrix):
            ...
            if self.balance:
                ...
                for i in range(self.n_classes_):
                    for j in range(self.n_classes_):
                        if i != j:
                            cost_matrix[i, j] = 1.0 / (class_counts[j] * self.n_classes_)
    ...
    def __create_reference_labels(self, label_binarizer_, y_ref):
        # Trasform y_ref into a binary matrix
        y_ref_bin = label_binarizer_.transform(y_ref) > 0.5
        (n, m) = y_ref_bin.shape
        ...
        return np.ascontiguousarray(y_ref_bin.astype(np.uint8))

    def __save_debug_state(self, X, y, y_ref):

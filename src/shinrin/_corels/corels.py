"""CorelsClassifier vendored from pycorels (corels/corels.py).

Adapted to bind against ``shinrin._native`` (PyO3 over the vendored C++
sources) instead of the upstream Cython ``_corels`` module.
"""
...
from shinrin._native import (
    corels_fit_wrap_begin as fit_wrap_begin,
    corels_fit_wrap_end as fit_wrap_end,
    corels_fit_wrap_loop as fit_wrap_loop,
    corels_predict_wrap as predict_wrap,
)
...
class CorelsClassifier:
    ...
    This class implements the CORELS algorithm, designed to produce
    human-interpretable, optimal rulelists for binary feature data and binary
    classification. As an alternative to other tree based algorithms such as
    CART, CORELS provides a certificate of optimality for its rulelist given a
    training set, leveraging multiple algorithmic bounds to do so.

    In order to run the algorithm, create an instance of the
    `CorelsClassifier` class, providing any necessary parameters in its
    constructor, and then call `fit` to generate a rulelist. `printrl` prints
    the generated rulelist, while `predict` provides classification
    predictions for a separate test dataset with the same features. To
    determine the algorithm's accuracy, run `score` on an evaluation dataset
    with labels. To save a generated rulelist to a file, call `save`. To load
    it back from the file, call `load`.
    ...
    --------
    >>> import numpy as np
    >>> from shinrin._corels import CorelsClassifier
    ...
    def fit(self, X, y, features=[], prediction_name="prediction"):
        ...
        if not isinstance(self.c, float):
            raise TypeError(
                "Regularization constant (c) must be a float, got: " + str(type(self.c))
            )
        if self.c < 0.0 or self.c > 1.0:
            raise ValueError(
                "Regularization constant (c) must be between 0.0 and 1.0, got: "
                + str(self.c)
            )
        if not isinstance(self.n_iter, int):
            raise TypeError(
                "Max nodes must be an integer, got: " + str(type(self.n_iter))
            )
        if self.n_iter < 0:
            raise ValueError("Max nodes must be positive, got: " + str(self.n_iter))
        if not isinstance(self.ablation, int):
            raise TypeError(
                "Ablation must be an integer, got: " + str(type(self.ablation))
            )
        if self.ablation > 2 or self.ablation < 0:
            raise ValueError(
                "Ablation must be between 0 and 2, inclusive, got: "
                + str(self.ablation)
            )
        if not isinstance(self.map_type, str):
            raise TypeError(
                "Map type must be a string, got: " + str(type(self.map_type))
            )
        if not isinstance(self.policy, str):
            raise TypeError("Policy must be a string, got: " + str(type(self.policy)))
        if not isinstance(self.verbosity, list):
            raise TypeError(
                "Verbosity must be a list of strings, got: " + str(type(self.verbosity))
            )
        if not isinstance(self.min_support, float):
            raise TypeError(
                "Minimum support must be a float, got: " + str(type(self.min_support))
            )
        if self.min_support < 0.0 or self.min_support > 0.5:
            raise ValueError(
                "Minimum support must be between 0.0 and 0.5, got: "
                + str(self.min_support)
            )
        if not isinstance(self.max_card, int):
            raise TypeError(
                "Max cardinality must be an integer, got: " + str(type(self.max_card))
            )
        if self.max_card < 1:
            raise ValueError(
                "Max cardinality must be greater than or equal to 1, got: "
                + str(self.max_card)
            )
        if not isinstance(prediction_name, str):
            raise TypeError(
                "Prediction name must be a string, got: " + str(type(prediction_name))
            )

        label = check_array(y, ndim=1)
        labels = np.stack([np.invert(label), label])
        samples = np.ascontiguousarray(check_array(X, ndim=2))
        ...
        if self.c < 1.0 / n_samples:
            warnings.warn(
                "Regularization parameter should not be less than 1/nsamples = "
                + str(1.0 / n_samples),
                RuntimeWarning,
                stacklevel=2,
            )
        ...
        if self.c > min(l_ones, n_samples - l_ones) / n_samples:
            warnings.warn(
                "Regularization parameter should not be greater than min(negative_examples, positive_examples) / n_samples = "
                + str(min(l_ones, n_samples - l_ones) / n_samples),
                RuntimeWarning,
                stacklevel=2,
            )
        ...
        if rl.features and len(rl.features) != n_features:
            raise ValueError(
                "Feature count mismatch between sample data ("
                + str(n_features)
                + ") and feature names ("
                + str(len(rl.features))
                + ")"
        ...
        if (
            ...
            raise ValueError(
                "'samples' verbosity option must be combined with at"
        ...
        verbose = ",".join(
            [v for v in self.verbosity if v != "rulelist" and v != "mine"]
        )
        ...
        fr = fit_wrap_begin(
            samples.astype(np.uint8, copy=False),
            np.ascontiguousarray(labels.astype(np.uint8, copy=False)),
            rl.features,
            self.max_card,
            self.min_support,
            verbose,
    ...
    def predict(self, X):
        ...
        if samples.shape[1] != len(self.rl_.features):
            raise ValueError(
                "Feature count mismatch between eval data ("
                + str(X.shape[1])
                + ") and feature names ("
                + str(len(self.rl_.features))
                + ")"
            )

        return np.array(
            predict_wrap(samples.astype(np.uint8, copy=False), self.rl_.rules),
            dtype=bool,
        )
    ...
    def score(self, X, y):
        ...
        if p.ndim == 2:
            ...
            raise ValueError(
                "Input samples must have only 1 or 2 dimensions, got "
                + str(p.ndim)
                + " dimensions"
            )
    ...
    def set_params(self, **params):
        """
        Set model parameters. Takes an arbitrary number of keyword parameters, all of
        which must be valid parameter names (i.e. must be included in those returned
        by get_params).
    ...
    def load(self, fname):
        ...
        with open(fname, "rb") as f:
            ...
            if not hasattr(model, "get_params"):
                raise ValueError(
                    "Invalid model provided, model must have get_params() method"
                )

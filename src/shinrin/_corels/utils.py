"""Helper utilities vendored from pycorels (corels/utils.py).

Adapted for NumPy >= 2: ``np.bool`` is gone and ``np.array(..., copy=False)``
no longer copies lazily, so ``asarray``/plain ``bool`` are used instead.
"""
...
def check_array(x, ndim=None):
    if (
        not hasattr(x, "shape")
        and (type(x) == str or not hasattr(x, "__len__"))
    ...
    if ndim and ndim != x.ndim:
        raise ValueError(
            "Array must be "
            + str(ndim)
            + "-dimensional in shape, got "
            + str(x.ndim)
            + " dimensions instead"
        )

    asbool = x.astype(bool)

    if not np.array_equal(x, asbool):
        raise ValueError(
            "Array must contain only binary members (0 or 1), got " + str(x)
        )
...
def check_in(name, allowed, val):
    if val.lower() not in allowed:
        ...
        raise ValueError(name + " must be chosen from " + allowed_str + ", got: " + val)
...
def check_features(features):
    ...
    for i in range(len(features)):
        if not isinstance(features[i], str):
            raise TypeError(
                "Each feature much be a string, got: " + str(type(features[i]))
            )


def check_rulelist(rl):
    if (
        not hasattr(rl, "rules")
        or not hasattr(rl, "features")
        or not hasattr(rl, "prediction_name")
    ):
        raise ValueError(
            "Rulelist must have the following attributes: 'rules', 'features', 'prediction_name'"
        )
    ...
    if not isinstance(rl.prediction_name, str):
        raise TypeError(
            "Prediction name must be a string, got: " + str(type(rl.prediction_name))
        )
    ...
    for r in range(len(rl.rules)):
        ...
        if r == (len(rl.rules) - 1) and (
            a_len != 1 or rl.rules[r]["antecedents"][0] != 0
        ):
            raise ValueError(
                "Last rule in the rulelist must be the default prediction,"
...
class RuleList:
    ...
    def save(self, fname):
        ...
        with open(fname, "wb") as f:
            pickle.dump(
                {"f": self.features, "r": self.rules, "p": self.prediction_name}, f
            )
    ...
    def load(self, fname):
        ...
        with open(fname, "rb") as f:
            ...
            if "r" not in rl_dict or "f" not in rl_dict or "p" not in rl_dict:
        ...
        if len(self.rules) == 1:
            ...
            for i in range(len(self.rules) - 1):
                ...
                for j in range(1, len(self.rules[i]["antecedents"])):
                    feat += " && " + get_feature(
                        self.features, self.rules[i]["antecedents"][j]
                    )
                tot += (
                    "if ["
                    + feat
                    ...
                    + " = "
                    + str(bool(self.rules[i]["prediction"]))
                    + "\nelse "
                )
            ...
            tot += (
                ...
                + self.prediction_name
                + " = "
                + str(bool(self.rules[-1]["prediction"]))
            )
...
def load_from_csv(fname):
    """
    Load a dataset from a csv file. The csv file must contain n_samples+1 rows,
    each with n_features+1 columns. The last column of each sample is its
    prediction class, and the first row of the file contains the feature names
    and prediction class name.
